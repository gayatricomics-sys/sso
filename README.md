...
The scanner includes an integrated **Executive Report Module** inside the `Executive Report` UI tab.

### Features of the Report Module:
1. **Real-time Live Preview**: Displays a formatted markdown preview of the audit findings, scope summary, and severity matrix.
2. **Professional HTML Report Export**: Click **Export Professional HTML Report** to generate a standalone, styled HTML document featuring:
   - Executive Risk Summary & Severity Breakdown Cards (High/Critical, Medium, Low, Informational).
   - Scope Matrix detailing auto-discovered endpoints and captured tokens.
   - Stage 1–4 Checklist Audit Status Matrix.
   - Detailed Findings Breakdown (Affected URLs, Risk Impact, and Remediation Guidance).
3. **Markdown Report Export**: Click **Export Markdown Report** to save a `.md` audit report for integration into technical documentation.
4. **Native Burp Suite HTML Report**: In Burp Pro/Community, findings are also exported via Burp's native **Dashboard / Issues** HTML report generator.
